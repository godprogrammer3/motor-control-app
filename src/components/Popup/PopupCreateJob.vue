      ...
      <v-container>
        <v-row>
          <v-date-picker
            ...
            width="35vw"
            locale="th"
            style="transform:scale(1.1);margin-left:18vw;margin-bottom:5vh;"
            :min="minDate"
          ></v-date-picker
        ></v-row>
...
export default {
  ...
  data() {
    const sevenHoursInMilliSecond = 1000*60*60*7;
    return {
      jobId: "",
      jobIdRules: [
        (v) => !!v || "กรุณากรอกหมายเลขงาน",
        (v) => v.length <= 20 || "ความยาวตัวอักษรสูงสุด 20 ตัว",
      ],
      width: "",
      ...
      workDate: this.parseDate(new Date(new Date().getTime()+sevenHoursInMilliSecond).toISOString().substr(0, 10)),
      workDateRules: [(v) => !!v || "กรุณากรอกวันที่ดำเนินงาน"],
      currentInput: "",
      valid: false,
      picker: new Date(new Date().getTime()+sevenHoursInMilliSecond).toISOString().substr(0, 10),
      minDate: new Date(new Date().getTime()+sevenHoursInMilliSecond).toISOString().substr(0, 10),
    ...
    async keyboardEventHandler(event) {
      if (event.type == "letter" && event.value != ".") {
        ...
        if (this.currentInput == "jobId") {
          ...
          if (
            this.jobId.length < 20 ||
            element.selectionStart != element.selectionEnd
          ) {
            var newSelectionStart = element.selectionStart + 1;
            this.jobId =
              this.jobId.substring(0, element.selectionStart) +
              event.value +
              this.jobId.substring(element.selectionEnd);
            this.$nextTick(() => {
              element.focus();
          ...
          if (
            ...
            var newSelectionStart = element.selectionStart + 1;
            this.width =
              this.width.substring(0, element.selectionStart) +
              event.value +
              this.width.substring(element.selectionEnd);
            this.$nextTick(() => {
              element.focus();
              element.setSelectionRange(newSelectionStart, newSelectionStart);
            });
          ...
          if (
            (this.height.length < 8 ||
              element.selectionStart != element.selectionEnd) &&
            (event.value != "0" || element.selectionStart != 0)
          ) {
            var newSelectionStart = element.selectionStart + 1;
            this.height =
              this.height.substring(0, element.selectionStart) +
              event.value +
              this.height.substring(element.selectionEnd);
            this.$nextTick(() => {
              element.focus();
              element.setSelectionRange(newSelectionStart, newSelectionStart);
            });
          ...
          if (
            ...
            var newSelectionStart = element.selectionStart + 1;
            this.sheet =
              this.sheet.substring(0, element.selectionStart) +
              event.value +
              this.sheet.substring(element.selectionEnd);
            this.$nextTick(() => {
              element.focus();
              element.setSelectionRange(newSelectionStart, newSelectionStart);
            });
        ...
        if (event.value == "delete") {
          if (this.currentInput == "jobId") {
            ...
            if (element.selectionStart == element.selectionEnd) {
              ...
              this.jobId =
                this.jobId.substring(0, element.selectionStart - 1) +
                this.jobId.substring(element.selectionEnd);
            } else {
            ...
            if (element.selectionStart == element.selectionEnd) {
              newSelectionStart = element.selectionStart - 1;
              this.width =
                this.width.substring(0, element.selectionStart - 1) +
                this.width.substring(element.selectionEnd);
            } else {
              newSelectionStart = element.selectionStart;
              this.width =
            ...
            if (element.selectionStart == element.selectionEnd) {
              newSelectionStart = element.selectionStart - 1;
              this.height =
                this.height.substring(0, element.selectionStart - 1) +
                this.height.substring(element.selectionEnd);
            } else {
              newSelectionStart = element.selectionStart;
              this.height =
            ...
            if (element.selectionStart == element.selectionEnd) {
              newSelectionStart = element.selectionStart - 1;
              this.sheet =
                this.sheet.substring(0, element.selectionStart - 1) +
                this.sheet.substring(element.selectionEnd);
            } else {
              newSelectionStart = element.selectionStart;
              this.sheet =
          ...
          if (this.$refs.form.validate()) {
            ...
            if (result.successful) {
              ...
              if (result.errorCode == "03") {
                this.dialogValue = {
                  errorMessage: "หมายเลขงานซ้ำกับที่มีอยู่ในระบบแล้ว",
                };
                this.isDialogShow = true;
              }
            }
