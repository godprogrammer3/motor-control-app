            ...
            <v-row align="center" justify="center">
              <v-form ref="form">
                <v-col>
                  ...
                  <v-text-field ref="wast" id="wast" suffix="แผ่น" v-model="wast" class="text-h4" placeholder="กรอกค่า"
    ...
    methods: {
      ...
      async confirmAddWastPaperEventHandler(event) {
        if (event == 'confirm') {
          ...
          let result = await API.controls.addWasteBySheet(Number(this.wast));
          console.log(result);
