...
export default {
      ...
      if (event.type == "letter" && event.value != ".") {
        ...
        if (event.value == "delete") {
          ...
          if (this.$refs.form.validate()) {
            this.overlay = true;
            const result = await API.jobs.update({
              id: this.jobId,
  ...
  mounted() {
    ...
    this.width = this.job.width.toString();
    this.height = this.job.height.toString();
    this.sheet = this.job.sheet.toString();
    this.workDate = this.parseDateFromDB(this.job.workDate);
  },
  watch: {
    job(newValue, oldValue) {
      ...
      if (newValue.width) {
        this.width = newValue.width.toString();
      }
      if (newValue.height) {
        this.height = newValue.height.toString();
      }
      if (newValue.sheet) {
        this.sheet = newValue.sheet.toString();
      }
      if (newValue.workDate) {
        this.workDate = this.parseDateFromDB(newValue.workDate);
      }
    },
  },
};
