...
var speed1 = 0;
(speed2 = 0), (SpeedTaget = 0), (speedSlow = 0);
...
socket1.on("connect", async function() {
...
const socket = (io) => {
  ...
  em.on("process", async () => {
    ...
    intervalProcess = setInterval(() => {
      ...
      if (
        Lenght1 > target - Number(process.env.LenghtDownSpeed) &&
        Lenght1 < target
      ) {
        SpeedTaget =
          Number(process.env.slowSpeed) +
          (speed1 * Number(process.env.xSlow) * (target - Lenght1)) /
            Number(process.env.LenghtDownSpeed);
        ...
        if (slow) {
          ...
          SpeedTaget =
            speed2 + (Lenght2 + onTop - Lenght1) * Number(process.env.xSpeed);
